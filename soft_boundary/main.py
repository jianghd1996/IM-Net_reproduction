...
class IM_Net(nn.Module):
    ...
    def testz(self, z, points):
        point_batch_size = points.shape[1]
        z = z.unsqueeze(1).repeat((1, point_batch_size, 1)).view(z.shape[0] * point_batch_size, -1)
    ...
    def __getitem__(self, index):
        ...
        if len(points) < self.pbz:
            for i in range(max(0, self.pbz - len(points))):
                points.append(np.random.randint(0, 64, 3))

        random.shuffle(points)
        points = np.array(points[:self.pbz])
        label = voxel[points[:, 0], points[:, 1], points[:, 2]]
...
class Agent(object):
    def __init__(self, data_path, category):
        ...
        self.optimizer = torch.optim.Adam(self.policies, lr=1e-4, betas=(0.9, 0.999),
    ...
    def save_ckpt(self, name=None):
        ...
        if name is None:
            save_path = os.path.join(self.model_dir, "ckpt_epoch%04d.pth"%self.clock.epoch)
    ...
    def visualization(self, number):
        ...
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        data_path = os.path.join(self.data_path, self.category)
        for i in range(number):
            file = os.path.join(data_path, name_list[i]['anno_id'] + ".h5")
            if not os.path.exists(file):
                continue

            with h5py.File(file, 'r') as fp:
                voxel = np.array(np.clip(fp["shape_voxel64"], 0, 1), dtype="float32")
            ...
            mesh.export(os.path.join(save_dir, "{}_smooth_result.obj".format(i)))
    ...
    def interpolation(self, A=None, B=None):
        ...
        with h5py.File(os.path.join(self.data_path, self.category, A), 'r') as fp:
            A = np.array(np.clip(fp["shape_voxel64"], 0, 1), dtype="float32")

        with h5py.File(os.path.join(self.data_path, self.category, B), 'r') as fp:
